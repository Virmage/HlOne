...
- **Storage method matches HyperLiquid's official frontend** (app.hyperliquid.xyz) — plaintext in browser storage is the industry-standard approach for HL agent wallets. HLOne uses the exact same pattern.
- **Used for:** signing individual HL orders without a wallet popup every trade.
- **Capabilities:** trade on your HL account. **Cannot withdraw, transfer, or modify account settings** — HL agents have constrained permissions enforced by the HL protocol itself, not by HLOne.
- **Revocation:** go to HyperLiquid → API → revoke agent.

### 3. Derive Session Key
- **Only exists if you explicitly import one.** HLOne never auto-generates Derive session keys. If you don't use the Derive options features, no Derive key ever gets created or stored.
- **Generated by you:** on derive.xyz (Settings → Developer → Session Keys). You copy the private key and paste it into HLOne.
- **Stored:** in `localStorage` under `hlone-derive-sk-<your-eoa>` — only after you've imported it.
...
- **Revocation:** go to derive.xyz → Settings → Developer → Session Keys → revoke. Revoking on-chain is the real safety net; clearing from HLOne's local storage alone doesn't invalidate a copy an attacker already took.
